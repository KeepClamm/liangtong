        ...
        tooltip: {
          ...
          formatter: (list) => {
            ...
            list.forEach(item => {
              ...
              // let p = `<br /> ${item.marker} ${item.seriesName}: ${item.data}${item.seriesName == '企业数量'? unit : item.seriesName == '企业数量累计占比' ? unit: ''}`;
              let p = `<br /> ${item.marker} ${item.seriesName}: ${item.data}${item.seriesName == '企业数量'? '' : item.seriesName == '企业数量累计占比' ? unit : ''}`;
              tooltip += p
            })
            return tooltip
          }
